<?php
/**
*	@version	$Id: helper.php 3 2014-10-15 08:26:48Z linhnt $
...
class ModK2LatestTimelineHelper
{
    public static function getAjax() {
        $input = JFactory::getApplication()->input;
        
        if ($input->get('cmd') == 'load') {
            $module = JModuleHelper::getModule('mod_k2latest_timeline', base64_decode($input->get('data')));
            ...
            if (count($items)) {
                ob_start();
                require (JModuleHelper::getLayoutPath('mod_k2latest_timeline', $getTemplate.DS.'default_ajax'));
	...
	public static function getItems(&$params, $format = 'html')
	{
		...
		if (count($items))
		{
            ...
            {
                $dispatcher->trigger('onContentPrepare', array('mod_k2latest_timeline.', &$item, &$params, $limitstart));
            }
				...
				if ($format != 'feed')
				{
					...
					if ($params->get('JPlugins', 1))
					{
						...
						if (K2_JVERSION != '15')
						{
							...
							$results = $dispatcher->trigger('onContentAfterTitle', array('mod_k2latest_timeline', &$item, &$params, $limitstart));
							$item->event->AfterDisplayTitle = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentBeforeDisplay', array('mod_k2latest_timeline', &$item, &$params, $limitstart));
							$item->event->BeforeDisplayContent = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentAfterDisplay', array('mod_k2latest_timeline', &$item, &$params, $limitstart));
					...
					foreach ($item->extra_fields as $key => $extraField)
					{
						if ($extraField->type == 'textarea' || $extraField->type == 'textfield')
						{
							...
							if ($params->get('JPlugins', 1))
							{
								if (K2_JVERSION != '15')
								{
									$dispatcher->trigger('onContentPrepare', array('mod_k2latest_timeline', &$tmp, &$params, $limitstart));
								}
								else
								{
